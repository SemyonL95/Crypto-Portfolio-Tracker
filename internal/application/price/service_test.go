...
func (m *mockProvider) setPrice(tokenAddress string, p *domainprice.Price) {
	m.prices[tokenAddress] = p
}
...
func TestService_GetPrices(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			...
			tokens: []*token.Token{
				{ID: "bitcoin", Symbol: "BTC", Address: "0xbtc"},
			},
			currency:        "USD",
			wantErr:         false,
			wantResultCount: 1,
		...
		},
		{
			...
			validateResult: func(t *testing.T, results map[*token.Token]*domainprice.Price, cache *mockCache, primary *mockProvider, fallback *mockProvider) {
				...
				cached, ok := cache.Get(context.Background(), "0xbtc:USD")
				if !ok {
		...
		},
		{
			...
			setupPrimary: func(p *mockProvider) {
				ethToken := &token.Token{ID: "ethereum", Symbol: "ETH", Address: "0xeth"}
				price := domainprice.NewPrice(ethToken, big.NewInt(300000000000), "USD")
				p.setPrice("0xeth", &price)
			},
			setupFallback: func(p *mockProvider) {
				// Should not be called
			},
			tokens: []*token.Token{
				{ID: "bitcoin", Symbol: "BTC", Address: "0xbtc"},
				{ID: "ethereum", Symbol: "ETH", Address: "0xeth"},
			},
			currency:        "USD",
			wantErr:         false,
			wantResultCount: 2,
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			service := NewService(cache, primary, fallback, nil, nil)
	...
	tests := []struct {
		...
		},
		{
			...
			tokens: []*token.Token{
				{ID: "bitcoin", Symbol: "BTC", Address: "0xbtc"},
				{ID: "ethereum", Symbol: "ETH", Address: "0xeth"},
				{ID: "solana", Symbol: "SOL", Address: "0xsol"},
			},
			currency:        "USD",
			wantErr:         false,
			wantResultCount: 0, // No results because provider is not called
		...
		},
		{
			...
			setupProvider: func(p *mockProvider) {
				btcToken := &token.Token{ID: "bitcoin", Symbol: "BTC", Address: "0xbtc"}
				ethToken := &token.Token{ID: "ethereum", Symbol: "ETH", Address: "0xeth"}
				solToken := &token.Token{ID: "solana", Symbol: "SOL", Address: "0xsol"}
				btcPrice := domainprice.NewPrice(btcToken, big.NewInt(5000000000000), "USD")
				...
				p.setPrice("0xbtc", &btcPrice)
				p.setPrice("0xeth", &ethPrice)
				p.setPrice("0xsol", &solPrice)
			},
			tokens: []*token.Token{
				{ID: "bitcoin", Symbol: "BTC", Address: "0xbtc"},
				{ID: "ethereum", Symbol: "ETH", Address: "0xeth"},
		...
		},
		{
			...
			setupProvider: func(p *mockProvider) {
				...
				for address, tokenID := range tokens {
					token := &token.Token{ID: tokenID, Symbol: tokenID[:3], Address: address}
					price := domainprice.NewPrice(token, big.NewInt(1000000000), "USD")
					p.setPrice(address, &price)
				}
			},
			tokens: []*token.Token{
				{ID: "bitcoin", Symbol: "BTC", Address: "0xbtc"},
				{ID: "ethereum", Symbol: "ETH", Address: "0xeth"},
				{ID: "solana", Symbol: "SOL", Address: "0xsol"},
				{ID: "cardano", Symbol: "ADA", Address: "0xada"},
				{ID: "polkadot", Symbol: "DOT", Address: "0xdot"},
			},
			currency:        "USD",
			wantErr:         false,
			wantResultCount: 5,
